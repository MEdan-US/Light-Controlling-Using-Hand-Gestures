...
## 🧠 Technical Overview
**Hand Landmark Detection**: The system uses MediaPipe Hands to extract 21 landmarks (each with X, Y, Z coordinates). This 63-dimensional vector is then flattened and fed into the classifier.

**Neural Network Architecture**: A Multi-Layer Perceptron (MLP) built with PyTorch:

-**Input**: 63 features.

-**Hidden Layers**: Four linear layers with 128 units, followed by ReLU activation, BatchNorm, and Dropout (0.4 - 0.6) to prevent overfitting.
